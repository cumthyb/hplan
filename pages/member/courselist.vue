	...
	<div class="my-course-panel">
				...
				<div slot="left" class="split-pane">
					<div>
						<div
							...
							@click="onSelectCourse(index)"
						>{{'课程'+(index+1)+':'+item.title}}</div>
					</div>
				</div>
				<div slot="right" class="split-pane course-detail">
					<div class="cmd">
					...
					<div class="video">
						<VedioPlayer v-if="currentCourse.videourl" :src="currentCourse.videourl[0]"/>
					</div>
				</div>
			</Split>
		</div>
		...
		>
			<div class="dotask-drawer-cmd">
				<Button style="margin-right: 8px" :disabled="quitDisable" @click="drawerShow = false">离开草稿</Button>
				<Button type="primary" v-if="modifyDisable" @click="onModifyTask">修改作业</Button>
				<Button type="primary" :disabled="submitDisable" @click="onSubmitTask">提交作业</Button>
			</div>
			<div class="dotask-drawer-task">
				<div class="task-title">{{currentTask.title}}</div>
				<div class="task-content">{{currentTask.desc}}</div>
			</div>
			<QuillEditor :editable="taskContentEdit" :value="taskContent" @editor-blur="onEditorBlur"/>
			<div v-if="!taskContentEdit" v-html="taskContent"></div>
		</Drawer>
	</div>
</template>
...
export default {
	...
	methods: {
		...
		//获取该学院该课程已提交的作业
		getPaperInfo(courseId) {
			this.$http.post('get-paper-ByCourse', { course: courseId }).then(r => {
		...
		onSubmitTask() {
			let parmas = {
				task: this.currentCourse._id,
				content: this.taskContent
			}
			this.currentPaper.content = this.taskContent
			this.$http.post('post-paper', this.currentPaper).then(r => {
				this.$Notice.success({
					title: '操作成功'
				})
